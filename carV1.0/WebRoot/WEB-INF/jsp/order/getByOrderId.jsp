...
function payStart(){
      var pay=document.getElementById("pay");
	   if(confirm("确定二维码支付?")){
	      pay.action="/car/order_payTrade.do";
	      pay.submit();  
	      } 
	      } 
</script>
		...
		<div class="span24">
			...
			<table>
				<tbody>
                             ...
                             <tr>
                                <th>汽车图片</th>
                                <td><a href="${ctx}/car_uCar.do?id=${order.car.id}"><img src="upload/${order.car.carImage}"/></a></td>
                              </tr> 
                             <tr>
                                <th>租车时间</th>
                             ...
                             <tr>
                               ...
                               <tr>
                                <th></th>
                                 <td>
                                   <a href="${ctx}/order_getByOrderId.do?id=${order.id}&flag=2" style="color: red">还车</a>
                                  </td>
                                </tr> 
                               </c:if>
                               <c:if test="${order.state==1}">
                               <tr>
                                <th></th>
                                 <td>
                                 <button type="submit" style="text-align:center;color: red;" onclick="payStart()">二维码付款</button>
                                   <%-- <a href="${ctx}/order_getByOrderId.do?id=${order.id}&flag=2" style="color: red">支付</a> --%>
                                  </td>
                                </tr> 
                               </c:if>
				</tbody>
		     	...
		     	<input type="hidden" name="dailyPrice" value="${order.car.dailyPrice}"/>
		     	<input type="hidden" name="carNumber" value="${rder.car.carNumber}"/>
				<input type="hidden" name="cname" value="${order.car.carCategory.cname}"/>
				<input type="hidden" name="rentTime" value="${order.rent.rentTime}"/>
				<input type="hidden" name="returnTime" value="${order.rent.returnTime}"/>
				</form>
		</div>
	</div>
